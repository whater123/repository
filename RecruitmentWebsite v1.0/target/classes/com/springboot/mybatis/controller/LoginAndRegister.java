...
public class LoginAndRegister {
    ...
    @RequestMapping(value = "/login/{remember}" , produces = "application/json;charset=UTF-8")
    public String login(@RequestBody String context,@PathVariable String remember, HttpServletRequest request, HttpServletResponse response) throws JsonProcessingException {
        //默认记住密码
        try{
            ...
            if (!loAndReService.canLogin(user)) {
                ...
                if ("1".equals(remember)){
                    Cookie number = new Cookie("number", user.getNumber());
                    Cookie password = new Cookie("password", user.getPassword());
                    number.setPath("/");
                    password.setPath("/");
                    number.setMaxAge(60 * 60 * 24 *10);
            ...
            return jsonUtil.getJson(new StateCode("-1","后端异常:"+e));
        }
    }
