...
public class ActivityPageController {
    ...
    public ReturnData getActivities(@RequestBody Activity activity) {
        try {
    ...
    public ReturnData getAllActivities(@RequestBody Activity activity) {
        try {
            List<Activity> activityByTheme = activityService.getActivityByTheme(activity.getThemeId());
            if (activityByTheme == null) {
                return new ReturnData("1", true, "不存在该主题");
    ...
    @RequestMapping(path = "/getRelation",produces = "application/json;charset=UTF-8")
    public ReturnData getRelation(@RequestBody Relation relation){
        try {
            Relation relation1 = activityService.getRelation(relation.getActivityId(), relation.getUserId());
            if (relation1 == null) {
                return new ReturnData("1", true, "事件id或用户id有误");
            } else {
                return new ReturnData("0", false, new Data(relation1));
            }
        }catch (Exception e){
            e.printStackTrace();
            return new ReturnData("500", true, "服务器错误");
